...
- Exact action: Have a reviewer unfamiliar with the implementation read the home, features, use-cases, pricing, and FAQ/contact content. Ask them to describe the service and list promised integrations and outcomes. Search the rendered pages for AI content generation, social publishing, revenue attribution, guaranteed response/performance, Gmail sync, calendar sync, and CRM replacement claims.
...
- Pass/fail: [ ] PASS [ ] FAIL
- Preconditions: Active test tenant, approved templates, owned test phone/email, Twilio inbound/status URLs, SendGrid sender, affirmative consent.
- Exact action: Trigger initial SMS; observe provider acceptance and signed delivery callback; reply inbound; reply ` STOP `; try manual/automatic SMS again. Send email and use its signed unsubscribe link twice; try automation again. Force provider rejection and retry exhaustion. Invoke the same Twilio callback twice and attempt two concurrent worker claims.
- Expected result: Initial SMS identifies client and includes STOP. States distinguish provider accepted/sent/delivered/failed. Inbound is preserved. STOP revokes SMS and cancels future SMS. Email unsubscribe is secure, idempotent, and blocks future governed email. Rejection shows `Failed`, creates an operator task, retries at most three times, callbacks do not downgrade terminal state, and no message sends twice.
- Evidence to retain: Message/provider IDs, status timeline, signed-callback HTTP results, consent revocation rows, operations tasks, worker claim log/query, screenshots showing Failed.
...
- Expected result: Totals reconcile exactly; failed/pending/skipped are not delivered; provider acceptance is not delivery; appointment metric is `Leads moved to Appointment Set during this period`; verified bookings remain unavailable; date range, timezone, sources, definitions, and limitations display. Non-admin scope includes only assigned leads.
