...
- Pass/fail: [ ] PASS [ ] FAIL
- Preconditions: Active controlled test tenant, approved templates, owned test email and optional phone, enabled-provider callbacks, affirmative consent, one selected and tested appointment provider, and active appointment CRM events. Email-only mode is valid while Twilio/A2P is unavailable.
- Exact action: Submit the controlled test lead and observe the first real email. Reply and verify the inbound message and AI reply. Agree to one exact future appointment time. Verify authoritative provider availability, the external provider record and invitation, the RealtyTechAI appointment, lead stage, agent notification, and CRM delivery. Replay the booking request, reschedule, cancel, and reconcile one external edit. Switch the default to another tested provider and prove the old appointment remains bound to its original provider. Use Take over and verify AI stops immediately. If SMS is enabled, also trigger SMS, observe the signed delivery callback, reply inbound, reply ` STOP `, and try manual/automatic SMS again. Use the signed email unsubscribe twice, force one provider rejection/retry exhaustion, replay a callback, and attempt concurrent worker claims. Complete `docs/appointment-provider-controlled-uat.md` for the active provider.
- Expected result: Each enabled channel operates independently. There is one authoritative external record and one internal appointment; they match after supported reschedule/cancel/reconciliation paths. No internal appointment exists when availability or external creation is uncertain. The lead reaches `appointment_set`, the assigned agent is notified, idempotent appointment CRM events are queued, and takeover cancels queued AI/follow-up. A default-provider switch affects only new bookings. SMS STOP and email unsubscribe affect the correct channel. Rejection is visible, bounded retries create an operator task, terminal states do not regress, and no message or booking occurs twice.
...
- Expected result: Totals reconcile exactly; failed/pending/skipped are not delivered; provider acceptance is not delivery; the reporting appointment metric remains `Leads moved to Appointment Set during this period`, while appointment records separately expose their authoritative provider and synchronization status; date range, timezone, sources, definitions, and limitations display. Non-admin scope includes only assigned leads.
