...
Prompt text is sent with `store: false` and is not persisted in assistant-run records. The run stores a digest, withheld-length preview, provider/model, structured actions, token counts, estimated cost, latency, error classification, actor, tenant, and audit evidence. Model output is strict JSON and can request only the exact allowlist; the application validates and executes it.
