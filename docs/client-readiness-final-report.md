...
| B4 — Stripe correctness | VERIFIED | `billing.service.ts`, `stripe-plan-config.ts`, `stripe-webhook-event.entity.ts`, tenant billing fields | Four explicit price mappings; transaction/advisory lock and Stripe lookup prevent duplicate subscriptions; signatures checked; events claimed idempotently; full state sync; failure/cancellation/unknown-price tasks. | Focused Stripe: 2 suites/7 tests. | Create live products/prices/webhook/portal/dunning/tax policy and complete sandbox/live reconciliation. |
...
| B14 — Isolation/credentials/CSP | VERIFIED | tenant-scoped lead/message/sequence/user/integration/stats services; integration encryption; CSP headers; migration FKs/uniques | Direct-ID operations enforce tenant predicates; provider credentials are AES-256-GCM encrypted/masked and tenant-owned; CSP/HSTS/security headers applied; plaintext credentials fail readiness. | Focused isolation: 5 suites/14 tests; operational-log redaction and migration constraints. | Production-like two-tenant matrix; remediate any legacy plaintext rows found. |
...
- Credential handling: tenant integration secrets use AES-256-GCM with a base64 key decoding to exactly 32 bytes, are masked in UI/API, unique per tenant/provider, and never fall back to global tenant messaging credentials. Readiness refuses legacy plaintext rows.
...
- Price mapping: exactly `STRIPE_PRICE_PRO_MONTH`, `STRIPE_PRICE_PRO_YEAR`, `STRIPE_PRICE_TEAMS_MONTH`, and `STRIPE_PRICE_TEAMS_YEAR`; unknown/missing price IDs fail closed and create high-priority work.
...
| Stripe account/catalog | Complete account/business/bank verification; create four approved recurring `price_…` objects; set Railway `STRIPE_SECRET_KEY=sk_live_…`, four `STRIPE_PRICE_*`, `STRIPE_WEBHOOK_SECRET=whsec_…`; portal, receipts, branding, descriptor, dunning, cancellation, and written Tax decision. | Legal/commercial/tax decisions and live Stripe access. | `/health/readiness` billing up; inspect four checkouts; portal/payment method; receipts/invoices; tax; no unknown mapping. | Wrong charge/plan/tax, blocked payout, entitlement mismatch. |
| Stripe webhook/UAT | Endpoint `<api-origin>/billing/webhook`; events `checkout.session.completed`, `customer.subscription.created|updated|deleted`, `invoice.payment_succeeded|failed`; perform sandbox checkout/duplicate/renewal/failure/cancel/refund and one approved low-value live payment. | Public API, live prices/secret, UAT tenant, owner approval. | Unique completed event ledger; one customer/subscription; tenant dates/status reconcile to Stripe/invoice/receipt/payout; retain IDs then follow refund plan. | Duplicate/missed/forged billing state or first-client mischarge. |
