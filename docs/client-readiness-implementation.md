...
| B4 | Stripe mapping, duplicate prevention, signed/idempotent sync | VERIFIED | One canonical monthly service price, advisory checkout lock, event ledger, stored-subscription compatibility, full state sync, unknown-price/payment/cancellation tasks | `billing.safety.spec.ts`, `billing.service.spec.ts` | Live service price, webhook, portal, dunning, tax, controlled payment |
