...
| [ ] | Railway health | Deployment healthcheck path `/health/live`; monitor `/health/readiness` separately and do not activate clients while it returns HTTP 503. | Database/migrations/config/system email/billing. | Liveness is HTTP 200; readiness is HTTP 200 `status: ready`, with no pending migrations or legacy plaintext credential rows before client activation. | A healthy process can be confused with a launch-ready platform. |
