...
| [ ] | Commercial decision record | Approve `Managed Pro Pilot` and `Managed Teams Pilot`, exact setup fee, monthly/annual amounts, currency, provider-cost pass-through/inclusion, message allowance, and overage handling. | Unit economics and Stripe products. | Signed price sheet reconciles to agreement, SOW, public presentation, and all four Stripe prices. | Misbilling, margin loss, or misleading public presentation. |
| [ ] | Commercial policy | Approve trial/no-trial rule, cancellation notice/effective-date rule, refund policy, and monthly versus annual renewal behavior. | Counsel/accountant and Stripe configuration. | Agreement, terms, refund page, Stripe portal, and UAT Tests 17–20 agree. | Disputes, refunds, or service continuing after cancellation. |
...
| [ ] | Stripe Products/Prices | Create recurring products/prices matching the approved Pro/Teams monthly/annual packages. Record four `price_…` IDs. | Approved package/price sheet and tax decision. | Price currency, amount, interval, tax behavior, and product name reconcile to the signed mapping. | Wrong plan, amount, interval, or entitlement. |
| [ ] | Railway variables | Set `STRIPE_SECRET_KEY=sk_live_…`, `STRIPE_PRICE_PRO_MONTH=price_…`, `STRIPE_PRICE_PRO_YEAR=price_…`, `STRIPE_PRICE_TEAMS_MONTH=price_…`, `STRIPE_PRICE_TEAMS_YEAR=price_…`; all must be from live mode. | Live products/prices. | Redeploy; `/health/readiness` reports billing up; start each checkout and inspect its line item before payment. | Checkout fails or an unknown price blocks service. |
| [ ] | Stripe Workbench → Webhooks | Endpoint URL exactly `<api-origin>/billing/webhook`. Subscribe only as needed to `checkout.session.completed`, `customer.subscription.created`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_succeeded`, and `invoice.payment_failed`. Set Railway `STRIPE_WEBHOOK_SECRET=whsec_…`. | Public Railway HTTPS origin and live Stripe account. | Send a Stripe test event; ledger row completes once; an invalid signature is rejected before ledger insertion. | Forged, missed, duplicated, or stale billing state. |
...
| [ ] | Twilio number | Assign one production number/sender to the intended tenant. Save Account SID, auth token, sender number, and optional Messaging Service SID through that tenant’s Integrations screen so storage is encrypted; do not set tenant credentials as global environment variables. | Approved registration and tenant authorization. | Integration UI masks values and reports connected after a safe test; credential row begins `v1:` and routing key is unique. | Cross-tenant routing or credential exposure. |
...
| [ ] | Railway health | Healthcheck path `/health/readiness`; optionally use `/health/live` for liveness. Treat HTTP 503 readiness as not deployable. | Database/migrations/config/system email/billing. | Response is HTTP 200 `status: ready`, with no pending migrations or legacy plaintext credential rows. | Traffic reaches an unsafe or incompatible process. |
