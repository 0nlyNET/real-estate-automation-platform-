...
assert.match(proxySource, /fetch\(`\$\{backend\}\/auth\/session`/)
...
assert.match(onboarding, /businessIdentity: data\.businessIdentity/)
assert.match(onboarding, /leadHandling: data\.leadHandling/)
assert.match(onboarding, /brandCommunication: data\.brandCommunication/)
assert.match(onboarding, /smsEnabled: false, \/\/ SMS channel disabled until separately ready/)
assert.match(onboarding, /emailEnabled: data\.emailEnabled/)
assert.match(onboarding, /bookingEnabled: data\.bookingEnabled/)
assert.match(onboarding, /targetLaunchDate: data\.targetLaunchDate \|\| null/)
assert.doesNotMatch(onboarding, /const normalized: RecordData = \{\s*\.\.\.data,/)
...
const sidebar = read("components/app-shell/sidebar.tsx") + read("lib/client-navigation.ts")
